...
const ProfileScreen = () => {
  ...
  const handleLike = (publicationId) => {
    const isLiked = publicationsData
  ...
  const handlePublish = () => {
    navigation.navigate("Post", {
      userName: user.nom,
      userProfilePhoto: user.profile_url,
    });
  };
      ...
      if (userId !== null) {
        try {
          const response = await axios.get(
            `${SERVER_IP}/api/users/${userId}/partage/`
          );
            ...
            <View key={index} style={styles.publicationContainer}>
              <View style={styles.personContainer}>
                ...
                <Text style={styles.personName}>
                  {publication.sender["nom"]}
                </Text>
                {publication.sender.id === userId && (
                  <TouchableOpacity
                    style={styles.optionsButton}
                    onPress={() => toggleOptions(publication.id)}
                  >
              ...
              {publication.images_list &&
                publication.images_list.length > 0 && (
                  <Image
              ...
              <View style={styles.interactionContainer}>
                <TouchableOpacity
                  ...
                  onPress={() => handleLike(publication.id)}
                >
                  <Ionicons
                    name="heart"
                    size={20}
                    color={
                      publication.likes.includes(userId) ? "#ff0000" : "#000000"
                    }
                  />
