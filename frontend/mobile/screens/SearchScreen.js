...
const SearchScreen = () => {
  ...
  const handleLike = (publicationId) => {
    const isLiked = publicationList
      .find((publication) => publication.id_partage === publicationId)
      ?.likes.includes(userId);
    setLikes((likes) => (isLiked ? likes - 1 : likes + 1));

    axios
      .put(`${SERVER_IP}/api/partage/likes/${publicationId}/${userId}/`)
      .then((response) => {
    ...
    setPublicationList((prevPublications) => {
      return prevPublications.map((publication) => {
        if (publication.id_partage === publicationId) {
          return {
            ...publication,
            likes_number: isLiked
              ? publication.likes_number - 1
              : publication.likes_number + 1,
            likes: isLiked
              ? publication.likes.filter((like) => like !== userId)
    ...
    try {
      ...
      if (userData) {
        ...
        const response = await axios.get(
          `${SERVER_IP}/api/partage/search/${id}/${query}/`
        );
  ...
  const handleSendMessage = async (userSelectId) => {
    try {
      ...
      if (userData && userSelectId) {
        ...
        const response = await axios.post(`${SERVER_IP}/api/discussion/`, {
          membres: [userId, userSelectId],
        });
        setDiscussionResult(response.data);

        navigation.navigate("DiscussionUsers", {
          userId,
          discussion: discussionResult.id_discussion,
          userDiscussion: discussionResult.nom,
          profile_url: discussionResult.profile_url,
        });
      }
    } catch (error) {
  ...
  const renderPublicationItem = ({ item }) => (
    <View style={styles.publicationContainer}>
      <TouchableOpacity
        onPress={() => handleProfile(item.sender.id)}
        style={styles.personContainer}
      >
        <Image
          source={{ uri: item.sender.profile_url }}
          style={styles.personImage}
        />
        <Text style={styles.personName}>{item.sender.nom}</Text>
      </TouchableOpacity>

      {item.contenu && <Text style={styles.title}>{item.contenu}</Text>}
      ...
      {item.files_list &&
        item.files_list.length > 0 &&
        item.files_list.map((file, fileIndex) => (
          <TouchableOpacity
      ...
      <View style={styles.interactionContainer}>
        <TouchableOpacity
          ...
          onPress={() => handleLike(item.id_partage)}
        >
          <Ionicons
            name="heart"
            size={20}
            color={
              item.likes.filter((l) => l == userId).length == 1
                ? "#ff0000"
                : "#000000"
            }
          />
        ...
        <TouchableOpacity
          ...
          onPress={() => handleComment(item.id_partage)}
        >
