...
function Comment() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${SERVER_IP}/api/partage/${publicationId}/comments/`
        );
  ...
  const updateCommentsLike = async (messageId) => {
    try {
      await axios.put(
        `${SERVER_IP}/api/discussion/messages/${messageId}/like/${userId}/`
      );
      setNewComment(" ");
      setNewComment("");
      print(userId);
      ...
      {publication && (
        <View style={styles.publicationContainer}>
          ...
          {publication.images_list && publication.images_list.length > 0 && (
            <Image
              source={{ uri: publication.images_list[0] }}
              style={styles.image}
            />
          )}
          {publication.files_list &&
            publication.files_list.length > 0 &&
            publication.files_list.map((file, index) => (
              <TouchableOpacity
          ...
          <View style={styles.interactionContainer}>
            <TouchableOpacity
              ...
              onPress={() => handleLike(publication.id_partage)}
            >
              <Ionicons
                name="heart"
                size={20}
                color={
                  publication.likes.filter((l) => l == userId).length == 1
                    ? "#ff0000"
                    : "#000000"
                }
              />
            ...
            <TouchableOpacity
              style={styles.reaction}
              onPress={() => handleAnswerComment(publication.id_partage)}
            >
              <Ionicons name="chatbubble-outline" size={20} color="black" />
              <Text style={styles.commentButtonText}>
                {publication.comments_number}{" "}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.reaction}
      ...
      <ScrollView style={styles.commentSection}>
        {commentsList.map((comment, index) => (
          <View key={index} style={styles.comment}>
            <Image
              source={{ uri: comment.sender["profile_url"] }}
              style={styles.personImage}
            ></Image>
            ...
            {userId && (
              <View style={styles.reactions}>
                <TouchableOpacity
                  ...
                  onPress={() => updateCommentsLike(comment.id_message)}
                >
                ...
                <TouchableOpacity
                  style={styles.reactionButton}
                  onPress={() => handleAnswerComment(comment.discussion)}
                >
